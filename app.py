...
from shapely.geometry import Point, LineString, Polygon, mapping as shapely_mapping
import simplekml
...
@app.route('/api/upload_geospatial_zip', methods=['POST'])
@limiter.limit("10 per minute;50 per hour")
def upload_geospatial_zip():
    """
    Handles uploading and processing of geospatial ZIP files.
    Extracts .shp and .geojson files, transforms them to WGS84,
    and returns their geometries and properties as GeoJSON.
    """
    if 'zipfile' not in request.files: # Check for 'zipfile'
        return jsonify({"status": "error", "message": "No 'zipfile' part in the request."}), 400

    file = request.files['zipfile'] # Use 'zipfile'
    if file.filename == '':
    ...
    # Secure the filename before using it, though for tempfile usage it's less critical
    # as we use the temp directory's generated name.
    # However, if we were to use file.filename directly in path construction later, this would be important.
    # For now, we'll use it for deriving layer names.
    original_zip_filename = secure_filename(file.filename)
    current_app.logger.info(f"Processing uploaded ZIP file: {original_zip_filename}")

    processed_layers_data = []

    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            zip_filepath = os.path.join(tmpdir, "uploaded.zip") # Temporary name for the zip itself
            file.save(zip_filepath)
            current_app.logger.info(f"Saved uploaded ZIP to temporary path: {zip_filepath}")

            extract_path = os.path.join(tmpdir, "extracted_files")
            os.makedirs(extract_path, exist_ok=True)
            current_app.logger.info(f"Extraction path: {extract_path}")

            try:
                with zipfile.ZipFile(zip_filepath, 'r') as zip_ref:
                    zip_ref.extractall(extract_path)
                current_app.logger.info(f"Successfully extracted ZIP contents to {extract_path}")
            except zipfile.BadZipFile:
                current_app.logger.error(f"BadZipFile error for {original_zip_filename}")
                ...
                current_app.logger.error(f"Error extracting ZIP {original_zip_filename}: {str(e_zip)}", exc_info=True)
                return jsonify({"status": "error", "message": f"Error processing ZIP file: {str(e_zip)}"}), 500


            # Walk through extracted files and process geospatial ones
            for root, _, files in os.walk(extract_path):
                for filename in files:
                    filepath = os.path.join(root, filename)
                    layer_name_base = os.path.splitext(filename)[0]
                    
                    # Process Shapefiles
                    if filename.lower().endswith('.shp'):
                        # Check for essential companion files
                        shx_file = filepath[:-4] + ".shx"
                        dbf_file = filepath[:-4] + ".dbf"
                        if not os.path.exists(shx_file):
                            current_app.logger.warning(f"Skipping SHP {filename}: missing .shx file.")
                            continue
                        if not os.path.exists(dbf_file):
                            # Geopandas can sometimes read SHP without DBF, but attributes will be missing.
                            # For consistency, we might choose to require it or log a more specific warning.
                            current_app.logger.warning(f"SHP file {filename} is missing its .dbf file. Attributes may be incomplete.")
                        
                        try:
                            current_app.logger.info(f"Reading SHP file: {filepath}")
                            gdf = gpd.read_file(filepath)
                            if gdf.empty or gdf.geometry.isnull().all(): # Check if empty or no valid geometries
                                current_app.logger.info(f"SHP file {filename} is empty or contains no valid geometries.")
                                continue

                            # CRS Handling and Transformation
                            if gdf.crs is None:
                                current_app.logger.warning(f"CRS for {filename} is undefined. Assuming EPSG:4326 (WGS84).")
                                # gdf.crs = "EPSG:4326" # Assign if assuming, or let to_crs handle it if it can infer
                                try:
                                     gdf_wgs84 = gdf.to_crs(epsg=4326) # Attempt transformation
                                except Exception as e_crs_transform_assume:
                                    current_app.logger.error(f"Failed to transform {filename} (assumed EPSG:4326) to EPSG:4326: {str(e_crs_transform_assume)}")
                                    continue # Skip this file if transformation fails
                            else:
                                try:
                                    gdf_wgs84 = gdf.to_crs(epsg=4326)
                                except Exception as e_crs_transform:
                                    current_app.logger.error(f"Failed to transform {filename} from {gdf.crs} to EPSG:4326: {str(e_crs_transform)}")
                            ...
                            for index, row in gdf_wgs84.iterrows():
                                if row.geometry is None or row.geometry.is_empty:
                                    continue
                                geom_geojson = shapely_mapping(row.geometry)
                                properties = row.drop('geometry').to_dict()
                                # Convert non-serializable types in properties
                                for key, value in properties.items():
                                    if isinstance(value, (pd.Timestamp, pd.Timedelta)):
                                        properties[key] = str(value)
                                    elif pd.isna(value) or value is pd.NaT:
                                        properties[key] = None
                                    elif hasattr(value, 'item'): # Handles numpy types like int64, float64
                                        properties[key] = value.item()
                                    elif isinstance(value, (Point, LineString, Polygon)): # Should not happen if 'geometry' is dropped
                                        properties[key] = shapely_mapping(value) # Convert other geometries if any in properties
                            ...
                            if layer_features:
                                processed_layers_data.append({
                                    "name": layer_name_base,
                                    "features": layer_features,
                                    "type": "vector" # Indicate it's vector data
                                })
                                current_app.logger.info(f"Successfully processed SHP layer: {layer_name_base} with {len(layer_features)} features.")

                        except Exception as e_gpd:
                            current_app.logger.error(f"Error reading or processing SHP file {filename}: {str(e_gpd)}", exc_info=True)
                            # Optionally, add a layer with an error status
                            continue # Skip this file on error


                    # Process GeoJSON files - Now ignored in ZIP upload
                    elif filename.lower().endswith('.geojson'):
                        current_app.logger.warning(
                            f"GeoJSON file '{filename}' found in ZIP archive. "
                            f"GeoJSON files within ZIPs are ignored. Please upload GeoJSON files directly."
                        )
                        continue # Skip GeoJSON files in ZIP

            # After iterating through all files
            if not processed_layers_data:
                return jsonify({"status": "success", "message": "ZIP file processed, but no valid Shapefiles (.shp) with geometries were found.", "layers": []}), 200

            return jsonify({
                "status": "success",
                "message": "Files processed successfully.",
                "layers": processed_layers_data
            }), 200

    except Exception as e:
        current_app.logger.error(f"Unhandled error in /api/upload_geospatial_zip: {str(e)}", exc_info=True)
        return jsonify({"status": "error", "message": "A server error occurred during file processing."}), 500


@app.route('/api/upload_geojson_file', methods=['POST'])
@limiter.limit("10 per minute;50 per hour")
def upload_geojson_file():
    """
    Handles uploading and processing of a single GeoJSON file.
    Transforms it to WGS84 and returns its geometries and properties.
    """
    if 'geojsonfile' not in request.files:
        return jsonify({"status": "error", "message": "No 'geojsonfile' part in the request."}), 400

    geojson_file = request.files['geojsonfile']

    if geojson_file.filename == '':
        return jsonify({"status": "error", "message": "No GeoJSON file selected."}), 400

    if not geojson_file or not geojson_file.filename.lower().endswith('.geojson'):
        return jsonify({"status": "error", "message": "Invalid file type. Please upload a GeoJSON file (.geojson)."}), 400

    filename = secure_filename(geojson_file.filename)
    current_app.logger.info(f"Processing uploaded GeoJSON file: {filename}")

    tmp_path = None # Initialize to ensure it's defined for finally block
    try:
        # Save to a temporary file for geopandas to read
        # Using NamedTemporaryFile with delete=False, then manually deleting.
        with tempfile.NamedTemporaryFile(suffix=".geojson", delete=False) as tmp:
            geojson_file.save(tmp.name)
            tmp_path = tmp.name
        
        current_app.logger.info(f"GeoJSON file saved to temporary path: {tmp_path}")
        gdf = gpd.read_file(tmp_path)

        if gdf.empty or gdf.geometry.isnull().all():
            current_app.logger.info(f"GeoJSON file '{filename}' is empty or contains no valid geometries.")
            return jsonify({"status": "error", "message": "GeoJSON file contains no valid geometries."}), 400

        # CRS Handling and Transformation
        if gdf.crs is None:
            current_app.logger.warning(f"CRS for GeoJSON '{filename}' is undefined. Assuming EPSG:4326 (WGS84).")
            # For GeoJSON, if CRS is None, it is typically assumed to be WGS84 (EPSG:4326) by convention.
            # Geopandas might already interpret it as such, but explicit transformation is safer.
            try:
                gdf_wgs84 = gdf.to_crs(epsg=4326)
            except Exception as e_crs_transform_assume:
                current_app.logger.error(f"Failed to transform GeoJSON '{filename}' (assumed EPSG:4326) to EPSG:4326: {str(e_crs_transform_assume)}")
                return jsonify({"status": "error", "message": f"CRS transformation error for '{filename}': {str(e_crs_transform_assume)}"}), 500
        elif str(gdf.crs).upper() != "EPSG:4326":
            current_app.logger.info(f"Transforming GeoJSON '{filename}' from {gdf.crs} to EPSG:4326.")
            try:
                gdf_wgs84 = gdf.to_crs(epsg=4326)
            except Exception as e_crs_transform:
                current_app.logger.error(f"Failed to transform GeoJSON '{filename}' from {gdf.crs} to EPSG:4326: {str(e_crs_transform)}")
                return jsonify({"status": "error", "message": f"CRS transformation error for '{filename}': {str(e_crs_transform)}"}), 500
        else:
            gdf_wgs84 = gdf # Already in WGS84
        ...
        for index, row in gdf_wgs84.iterrows():
            ...
            geom_geojson = shapely_mapping(row.geometry)
            properties = {}
            for col, value in row.drop('geometry').items():
                if pd.isna(value) or value is pd.NaT:
                    properties[col] = None
                elif isinstance(value, (pd.Timestamp, pd.Timedelta)):
                    properties[col] = value.isoformat() # Use isoformat for consistency
                elif hasattr(value, 'item'): # Handles numpy types like int64, float64
                    properties[col] = value.item()
                # Add more type checks here if necessary (e.g., for other complex types)
                else:
                    properties[col] = value
            
            features_for_layer.append({
                "type": "Feature",
                "geometry": geom_geojson,
                "properties": properties
            })
        
        if not features_for_layer: # Should be caught by gdf.empty earlier, but as a safeguard
            current_app.logger.info(f"No features extracted from GeoJSON file '{filename}' after processing.")
            return jsonify({"status": "error", "message": "No features could be extracted from the GeoJSON file."}), 400

        processed_layer_data = {
            "name": os.path.splitext(filename)[0], # Use filename without extension as layer name
            "features": features_for_layer,
            "type": "vector" # To match the structure of the ZIP upload response
        }
        
        current_app.logger.info(f"Successfully processed GeoJSON file: {filename} with {len(features_for_layer)} features.")
        return jsonify({
            "status": "success",
            "message": "GeoJSON file processed successfully.",
            "layers": [processed_layer_data] # Return as a list with one layer
        }), 200

    except Exception as e:
        current_app.logger.error(f"Unhandled error processing GeoJSON file '{filename}': {str(e)}", exc_info=True)
        return jsonify({"status": "error", "message": "A server error occurred during GeoJSON file processing."}), 500
    finally:
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
                current_app.logger.info(f"Successfully removed temporary file: {tmp_path}")
            except Exception as e_remove:
                current_app.logger.error(f"Error removing temporary file {tmp_path}: {str(e_remove)}")


@app.route('/api/upload_dxf_file', methods=['POST'])
@limiter.limit("10 per minute;50 per hour")
def upload_dxf_file():
    """
    Handles uploading and processing of a single DXF file.
    Extracts geometries from LINE, LWPOLYLINE, POLYLINE, CIRCLE, and TEXT entities.
    Returns data in a GeoJSON-like structure. Coordinates are transformed to WGS84 (EPSG:4326).
    """
    # 1. Receive and validate source_crs_epsg
    source_crs_epsg_str = request.form.get('source_crs_epsg')
    if not source_crs_epsg_str:
        return jsonify({"status": "error", "message": "Source CRS for DXF not provided."}), 400
    
    try:
        source_crs_epsg_int = int(source_crs_epsg_str)
    except ValueError:
        return jsonify({"status": "error", "message": "Invalid Source CRS EPSG code format."}), 400

    if 'dxffile' not in request.files:
        return jsonify({"status": "error", "message": "No 'dxffile' part in the request."}), 400

    dxf_file = request.files['dxffile']

    if dxf_file.filename == '':
        return jsonify({"status": "error", "message": "No DXF file selected."}), 400

    if not dxf_file or not dxf_file.filename.lower().endswith('.dxf'):
        return jsonify({"status": "error", "message": "Invalid file type. Please upload a DXF file (.dxf)."}), 400

    filename = secure_filename(dxf_file.filename)
    current_app.logger.info(f"Processing uploaded DXF file: {filename}")

    tmp_dxf_path = None
    try:
        with tempfile.NamedTemporaryFile(suffix=".dxf", delete=False) as tmp:
            dxf_file.save(tmp.name)
            tmp_dxf_path = tmp.name
        
        current_app.logger.info(f"DXF file saved to temporary path: {tmp_dxf_path}")

        doc = ezdxf.readfile(tmp_dxf_path)
        msp = doc.modelspace()
        
        # 2. Collect shapely geometries and properties
        shapely_geometries = []
        feature_properties = []

        # Extract LINE entities
        for line_entity in msp.query('LINE'):
            try:
                start_pt_coords = (line_entity.dxf.start.x, line_entity.dxf.start.y)
                end_pt_coords = (line_entity.dxf.end.x, line_entity.dxf.end.y)
                shapely_geometries.append(LineString([start_pt_coords, end_pt_coords]))
                feature_properties.append({"dxf_entity_type": "LINE", "layer": line_entity.dxf.layer})
            except Exception as e_line:
                current_app.logger.warning(f"DXF: Skipping LINE entity due to error: {str(e_line)} on layer {line_entity.dxf.layer}")

        # Extract LWPOLYLINE entities
        for lwpoly_entity in msp.query('LWPOLYLINE'):
            try:
                points_coords = [(p[0], p[1]) for p in lwpoly_entity.get_points(format='xy')]
                if len(points_coords) < 2:
                    continue
                is_closed = lwpoly_entity.is_closed
                if is_closed and (len(points_coords) < 3 or points_coords[0] != points_coords[-1]):
                    points_coords.append(points_coords[0])
                
                shapely_geometries.append(LineString(points_coords))
                feature_properties.append({"dxf_entity_type": "LWPOLYLINE", "layer": lwpoly_entity.dxf.layer, "closed": is_closed})
            except Exception as e_lwpoly:
                 current_app.logger.warning(f"DXF: Skipping LWPOLYLINE entity due to error: {str(e_lwpoly)} on layer {lwpoly_entity.dxf.layer}")

        # Extract POLYLINE entities (2D only)
        for poly_entity in msp.query('POLYLINE'):
            try:
                if poly_entity.is_3d_polyline or poly_entity.is_polygon_mesh:
                    continue
                points_coords = [(p.dxf.location.x, p.dxf.location.y) for p in poly_entity.vertices]
                if len(points_coords) < 2:
                    continue
                is_closed = poly_entity.is_closed
                if is_closed and (len(points_coords) < 3 or points_coords[0] != points_coords[-1]):
                    points_coords.append(points_coords[0])

                shapely_geometries.append(LineString(points_coords))
                feature_properties.append({"dxf_entity_type": "POLYLINE", "layer": poly_entity.dxf.layer, "closed": is_closed})
            except Exception as e_poly:
                current_app.logger.warning(f"DXF: Skipping POLYLINE entity due to error: {str(e_poly)} on layer {poly_entity.dxf.layer}")

        # Extract CIRCLE entities (center point)
        for circle_entity in msp.query('CIRCLE'):
            try:
                center_pt_coords = (circle_entity.dxf.center.x, circle_entity.dxf.center.y)
                radius = circle_entity.dxf.radius
                shapely_geometries.append(Point(center_pt_coords))
                feature_properties.append({
                    "dxf_entity_type": "CIRCLE_CENTER", 
                    "radius": radius, 
                    "layer": circle_entity.dxf.layer
                })
            except Exception as e_circle:
                current_app.logger.warning(f"DXF: Skipping CIRCLE entity due to error: {str(e_circle)} on layer {circle_entity.dxf.layer}")

        # Extract TEXT entities
        for text_entity in msp.query('TEXT'):
            try:
                insert_pt_coords = (text_entity.dxf.insert.x, text_entity.dxf.insert.y)
                shapely_geometries.append(Point(insert_pt_coords))
                feature_properties.append({
                ...
                current_app.logger.warning(f"DXF: Skipping TEXT entity due to error: {str(e_text)} on layer {text_entity.dxf.layer}")

        if not shapely_geometries:
            return jsonify({"status": "success", "message": f"DXF file '{filename}' processed, but no supported entities were found or extracted.", "layers": []}), 200

        # 3. Create GeoDataFrame and Transform
        props_df = pd.DataFrame(feature_properties)
        gdf = gpd.GeoDataFrame(props_df, geometry=shapely_geometries)

        try:
            gdf.set_crs(epsg=source_crs_epsg_int, inplace=True) 
            gdf_wgs84 = gdf.to_crs(epsg=4326)
        except Exception as e_crs:
            current_app.logger.error(f"CRS transformation error for DXF '{filename}' with source EPSG {source_crs_epsg_int}: {str(e_crs)}", exc_info=True)
            return jsonify({"status": "error", "message": f"Failed to transform DXF coordinates: {str(e_crs)}"}), 500

        # 4. Reconstruct layer_features with Transformed Geometries
        final_layer_features = []
        for index, row in gdf_wgs84.iterrows():
            transformed_geom_geojson = shapely_mapping(row.geometry)
            props = row.drop('geometry').to_dict()
            
            # Ensure properties are JSON serializable (similar to GeoJSON upload)
            for key, value in props.items():
                if pd.isna(value) or value is pd.NaT:
                    props[key] = None
                elif isinstance(value, (pd.Timestamp, pd.Timedelta)):
                    props[key] = value.isoformat()
                # Numpy types are usually handled by jsonify, but explicit conversion is safer if issues arise.
                # DXF properties here are mostly strings, numbers, or booleans from the original extraction.
                # The main concern would be if any properties were added that are complex types.

            final_layer_features.append({
                "type": "Feature",
                "geometry": transformed_geom_geojson,
                "properties": props 
            })

        processed_layer_data = {
            "name": os.path.splitext(filename)[0],
            "features": final_layer_features,
            "type": "vector-dxf" 
        }
        
        current_app.logger.info(f"Successfully processed and transformed DXF file: {filename} with {len(final_layer_features)} features.")
        return jsonify({
            "status": "success",
            "message": "DXF file processed successfully.",
            "layers": [processed_layer_data]
        }), 200

    except ezdxf.errors.DXFStructureError as e_dxf_struct:
        current_app.logger.error(f"DXFStructureError for '{filename}': {str(e_dxf_struct)}", exc_info=True)
        return jsonify({"status": "error", "message": f"Invalid or corrupted DXF file structure: {str(e_dxf_struct)}"}), 400
    except Exception as e:
        current_app.logger.error(f"Unhandled error processing DXF file '{filename}': {str(e)}", exc_info=True)
        return jsonify({"status": "error", "message": "A server error occurred during DXF file processing."}), 500
    finally:
        if tmp_dxf_path and os.path.exists(tmp_dxf_path):
            try:
                os.remove(tmp_dxf_path)
                current_app.logger.info(f"Successfully removed temporary DXF file: {tmp_dxf_path}")
            except Exception as e_remove:
                current_app.logger.error(f"Error removing temporary DXF file {tmp_dxf_path}: {str(e_remove)}")
