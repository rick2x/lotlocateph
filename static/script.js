    $(document).ready(function() {
        ...
        if (typeof $.fn.select2 === 'function') {
            $('#reference_point_select').select2({
                placeholder: "Select Reference Point", allowClear: true, dropdownAutoWidth: true,
                width: '100%', theme: "default" 
        ...
        function formatDataLine(ns, deg, min, ew, dist) { 
            ...
            return `${ns} ${degStr}D ${minStr}${primeSymbol} ${ew};${distStr};90`;
        }
        ...
        function addLot() {
            ...
            const listItem = $(`<li data-lot-id="${newLotId}"><span class="lot-name-display">${defaultLotName}</span><button class="btn-remove-lot-list" title="Remove ${defaultLotName}">×</button></li>`);
        ...
        function removeLot(lotIdToRemove) {
            ...
            if (plottedFeatureGroups[lotIdToRemove]) {
                if (map.hasLayer(plottedFeatureGroups[lotIdToRemove])) map.removeLayer(plottedFeatureGroups[lotIdToRemove]);
            ...
            if (activeLotId === lotIdToRemove) {
                ...
                else triggerMapUpdateWithDebounce(); 
            } else {
                 triggerMapUpdateWithDebounce();
            }
        }
        ...
        function setActiveLot(lotIdToActivate) {
            if (!lotIdToActivate || !surveyLotsStore[lotIdToActivate]) {
                 ...
                 if (lotListUL.find('li.active').length === 0 && lotListUL.find('li').length > 0) {
                    ...
                    if (firstLotId) lotIdToActivate = firstLotId; else return; 
                 } else if (!lotListUL.find('li').length) { return; } 
                 else if (!lotIdToActivate) { return; }
                 if (!surveyLotsStore[lotIdToActivate]) return;
            }
        ...
        function renderActiveLotEditor() {
                ...
                <div class="survey-point-editor">
                    ...
                    <div class="survey-point-table-content">
                        <div class="point-headers">
                            <span class="header-label">Line</span> <span class="header-ns">N/S</span>
                            <span class="header-deg">Deg</span> <span class="header-min">Min</span>
                            <span class="header-ew">E/W</span> <span class="header-dist">Dist (m)</span>
            ...
            if (lines.length > 0) {
                lines.forEach(line => {
                    ...
                    if (bearingDistance) addSurveyPointRowToActiveEditor(surveyPointsListContainer, bearingDistance);
                });
            } else { 
        ...
        // --- SURVEY POINT ROW MANAGEMENT (for active lot) ---
        function addSurveyPointRowToActiveEditor(surveyPointsListContainer, data = null) {
            ...
            const newRowHtml = `
                <div class="survey-point-row" data-row-index="${lotSpecificPointIndex}">
                    ...
                    <select class="point-input point-ns" name="${inputNamePrefix}_ns"> <option value="N" ${nsVal === 'N' ? 'selected' : ''}>N</option> <option value="S" ${nsVal === 'S' ? 'selected' : ''}>S</option> </select>
                    ...
                    <select class="point-input point-ew" name="${inputNamePrefix}_ew"> <option value="E" ${ewVal === 'E' ? 'selected' : ''}>E</option> <option value="W" ${ewVal === 'W' ? 'selected' : ''}>W</option> </select>
                    ...
                    <div class="btn-remove-point-container"> <button type="button" class="btn-remove-point" title="Remove this point">×</button> </div>
                </div>`;
        ...
        function updateRowLabelsForActiveEditor(surveyPointsListContainer) { 
            ...
            surveyPointsListContainer.children('.survey-point-row').each(function(index) {
                const pointRow = $(this);
                pointRow.find('.point-label').text(`Line ${index + 1}`);
                pointRow.attr('data-row-index', index + 1);
                const inputNamePrefix = `${activeLotId}_line${index + 1}`;
        ...
        lotListUL.on('click', 'li', function() { setActiveLot($(this).data('lot-id')); });
        ...
        function displayMessage(type, message) { 
            formMessagesArea.html(`<div class="message ${type}">${message}</div>`);
        }
        function clearMessages() { formMessagesArea.empty(); }

        function validateSurveyInputs(showAlerts = true, eventTargetId = null) { 
            let formIsValid = true; 
            if ($('#target_crs_select').val() === "") { 
                if (showAlerts) displayMessage('error', "CRS not selected."); formIsValid = false;
            }
            const lotsForPayload = getLotsForPayload();
            if ($('#reference_point_select').val() === "" && lotsForPayload.length > 0 && lotsForPayload.some(l => l.lines_text.trim() !== "")) { 
                 if (showAlerts) displayMessage('error', "Please select a Reference Point when lot data exists."); formIsValid = false;
            }
                ...
                if (surveyLotsStore.hasOwnProperty(lotId)) {
                    ...
                    if(lines.length > 0) { 
                        lines.forEach((line, index) => {
                            const parts = line.split(';');
                            if(parts.length < 2) { if (firstErrorLotName === null) { firstErrorLotName = lot.name; firstErrorLotLine = index + 1;} formIsValid = false; return; }
                            const bearingData = parseBearing(parts[0]);
                            const dist = parseFloat(parts[1]);
                            if (!bearingData || isNaN(dist) || dist <=0 || bearingData.deg < 0 || bearingData.deg > 89 || bearingData.min < 0 || bearingData.min > 59) {
                                if (firstErrorLotName === null) { firstErrorLotName = lot.name; firstErrorLotLine = index + 1;}
                                formIsValid = false;
                            }
                        });
                    }
            ...
            const isExportAction = (targetId) => targetId === 'exportShapefileBtn' || targetId === 'exportKmzBtn';
            if (isExportAction(eventTargetId) && lotsForPayload.filter(l => l.lines_text.trim() !== "").length === 0 && !$('#reference_point_select').val()) {
                if(showAlerts) displayMessage('error', "Cannot export: No reference point selected and no lot data with lines provided.");
                formIsValid = false;
            } else if (isExportAction(eventTargetId) && lotsForPayload.filter(l => l.lines_text.trim() !== "").length === 0 && $('#reference_point_select').val()) {
                if(showAlerts) displayMessage('info', "No lot data with lines. Export will only include the reference point if applicable.");
            }
            return formIsValid;
        }
        ...
        function getLotsForPayload() { 
            ...
            for (const lotId in surveyLotsStore) {
                if (surveyLotsStore.hasOwnProperty(lotId)) {
                    const lot = surveyLotsStore[lotId];
                    if (lot.lines_text.trim() !== "" || lot.name.trim() !== `Lot ${lot.num}` || Object.keys(surveyLotsStore).length === 1) {
        ...
        function fetchAndPlotMapData() { 
            clearMessages(); 
            if (!validateSurveyInputs(false, null)) { 
                displayMessage('warning', "Inputs invalid or incomplete. Map not updated.");
                return; 
            }
            ...
            if (!payload.reference_point_select && !hasLotsWithLines && Object.keys(surveyLotsStore).length === 0 ) { 
                ...
                if (result.status === 'success' || result.status === 'success_with_errors') {
                    ...
                    if (result.status === 'success_with_errors') {
                        ...
                        (result.data_per_lot || []).forEach(lr => { 
                            if(lr.status === 'error') errorMessages.push(`Lot '${lr.lot_name}': ${lr.message}`);
                            else if(lr.status === 'nodata' && payload.lots.find(l => l.id === lr.lot_id && l.lines_text.trim() !== "")){
                                 errorMessages.push(`Lot '${lr.lot_name}': No plottable geometry from provided lines.`);
                            }
                        });
                        if (errorMessages.length > 1) displayMessage('warning', errorMessages.join('<br>'));
                        else displayMessage('info', 'Map updated.'); 
                    } else {
            ...
            .catch(error => { 
                clearMessages(); console.error('Plot Fetch Error:', error);
        ...
        function plotMultiLotDataOnMap(dataPerLot, referencePlotData) { 
            ...
            if (referencePlotData && referencePlotData.reference_marker_latlng) {
                mainReferenceMarker = L.marker(referencePlotData.reference_marker_latlng, {title: "Reference Point"}).bindPopup("Reference Point");
            ...
            (dataPerLot || []).forEach((lotResult, index) => {
                ...
                if (lotResult.status !== 'success') { 
                    if (plottedFeatureGroups[lotId] && map.hasLayer(plottedFeatureGroups[lotId])) {
                        map.removeLayer(plottedFeatureGroups[lotId]);
                        delete plottedFeatureGroups[lotId]; 
                    }
                    return; 
                }
                ...
                if (plotData.tie_line_latlngs && plotData.tie_line_latlngs.length > 0) {
                    const tieLine = L.polyline(plotData.tie_line_latlngs, { color: '#e60000', dashArray: '6, 6', weight: 2.5, opacity: 0.8 }).bindPopup(`Tie-Line for ${lotName}`);
                ...
                if (plotData.parcel_polygon_latlngs && plotData.parcel_polygon_latlngs.length > 0) {
                    ...
                    const parcelPolyline = L.polyline(parcelPath, { color: color, weight: 3.5, opacity: 0.9 }).bindPopup(`Boundary: ${lotName}`);
                    ...
                    parcelPath.forEach((point, pIndex) => {
                        ...
                        if (pIndex === parcelPath.length - 1 && pIndex > 0 && 
                            ...
                            const vertexMarker = L.circleMarker(point, {radius: 4.5, color: color, weight:1, fillColor: color, fillOpacity: 0.7, title: label}).bindPopup(label);
        ...
        function handleFileDownload(response, defaultFilename, successMessage) { 
             if (!response.ok) {
        ...
        $('#exportShapefileBtn').on('click', function(e) {
            ...
            if (!payload.reference_point_select && payload.lots.filter(l => l.lines_text.trim() !== "").length === 0) {
                displayMessage('error', "Select Reference Point or add Lot Data for export."); return;
            }
            if (payload.lots.filter(l => l.lines_text.trim() !== "").length > 0 && !payload.reference_point_select) {
                displayMessage('error', "Reference Point must be selected to export lot data."); return;
            }
            displayMessage('info', 'Preparing Shapefile export...');
            fetch('/export_shapefile_multi', { 
                method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
            })
            .then(response => handleFileDownload(response, 'survey_export_multi.zip', 'Shapefile export'))
            .then(({blob, filename}) => { triggerDownload(blob, filename, 'Shapefile layers'); })
            .catch(error => {
                clearMessages(); console.error('Shapefile Export Error:', error);
        ...
        $('#exportKmzBtn').on('click', function(e) {
            ...
            .then(response => handleFileDownload(response, 'survey_export_multi.kmz', 'KMZ export'))
            .then(({blob, filename}) => { triggerDownload(blob, filename, 'KMZ file'); })
            .catch(error => {
                clearMessages(); console.error('KMZ Export Error:', error);
